...
video_capture = cv2.VideoCapture(0)  # Mở webcam
...
if Arguments.get("run_only_on_notification"):
    stdin_monitoring_thread = threading.Thread(
        target=monitor_stdin_for_command, daemon=True
    )
    stdin_monitoring_thread.start()
    Print.printJson(
        "status", "Started stdin monitoring thread for triggering face recognition."
    )
...
while True:
    ...
    if Arguments.get("run_only_on_notification"):
        if run_face_recognition == False and external_trigger == True:
            ...
            Print.printJson(
                "status",
                "Stopping face recognition and logging out any logged in users.",
            )
            run_face_recognition = False
            # picam2.stop()
            video_capture.release()  # Giải phóng camera khi không dùng
    ...
    if run_face_recognition:
        ...
        if not ret:
            continue  # Nếu không lấy được ảnh, bỏ qua vòng lặp này
        ...
        if logins.__len__() > 0:
            ...
            # Nếu Hiển đăng nhập, dừng camera ngay
            if "hien" in logins:
                Print.printJson("status", "Hien logged in. Stopping face recognition.")

                # Giải phóng camera
                video_capture.release()
                cv2.destroyAllWindows()

                # Thoát vòng lặp nhận diện
                break  # Thoát vòng lặp nhận diện
